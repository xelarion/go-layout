...
func initApp(cfg *config.Config, logger *zap.Logger, enableScheduler, enablePoller, enableQueue *bool) (*app.App, error) {
	...
	if *enableScheduler {
		...
		// Register all scheduler tasks
		if err := schedulerTasks.RegisterAll(taskScheduler, logger); err != nil {
	...
	if *enablePoller {
		...
		// Register all poller tasks
		if err := pollerTasks.RegisterAll(taskPoller, logger); err != nil {
	...
	if *enableQueue && queueManager != nil {
		// Register all queue tasks
		if err := queueTasks.RegisterAll(queueManager, logger); err != nil {
